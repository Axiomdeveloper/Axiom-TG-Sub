...
const CLIENTS = [
  ...
  { name: "V2Box", platform: "iOS" },
  { name: "Shadowrocket", platform: "iOS" },
];
...
export default function CopySub() {
  ...
  return (
    <section id="connect" className="relative z-10 mx-auto w-full max-w-6xl px-4 py-24 md:px-6">
      <SectionHeading
        kicker="// CONNECT"
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        <Reveal className="lg:col-span-3">
          <div className="glass g-border card h-full rounded-3xl p-6 md:p-8">
            ...
            <div
              dir="ltr"
              className="mt-4 flex items-center gap-2 overflow-x-auto rounded-2xl border hairline bg-black/40 px-4 py-3.5 font-mono text-[13px] text-emerald-100/90"
            >
              <span className="shrink-0 text-mint">➜</span>
              <code className="whitespace-nowrap">{subUrl || "…"}</code>
            </div>

            <div className="mt-5 flex flex-wrap items-center gap-2.5">
              <button onClick={copy} className="btn btn-mint">
                {copied ? <Check size={15} /> : <Copy size={15} />}
                {copied ? "کپی شد" : "کپی لینک"}
              </button>
              <button onClick={() => setShowQr((v) => !v)} className="btn btn-ghost">
                <QrCode size={15} />
              ...
              <div className="overflow-hidden">
                <div className="flex items-center gap-5">
                  ...
                  <p className="max-w-xs text-xs leading-6 text-zinc-400">
                    در v2rayNG گزینه‌ی <b className="text-zinc-200">اسکن QR</b> را در صفحه‌ی Subscription بزنید تا ساب
                    مستقیم اضافه شود.
                  </p>
                </div>
              </div>
        ...
        <Reveal delay={120} className="lg:col-span-2">
          <div className="glass rounded-3xl border hairline p-6 md:p-8 h-full">
            <p className="text-sm font-bold text-zinc-200">کلاینت‌های پیشنهادی</p>
            <ul className="mt-5 space-y-3">
              {CLIENTS.map((c) => (
                <li key={c.name} className="flex items-center justify-between gap-3 rounded-2xl border hairline bg-black/20 px-4 py-2.5">
